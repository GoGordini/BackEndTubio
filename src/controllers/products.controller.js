...
export const createProduct= async (req,res)=>{
        ...
        if (req.user.role!="admin" && owner!=req.user.email && owner!=null){
            // throw CustomError.createError({
            //     name: 'ProductError',
            ...
            //     code: EErrors.INVALID_TYPE_ERROR
            // })
            return res.status(422).send({status:"error", message:"Owner should be your email address"});
        }
        ...
        if (!result) {return res.status(404).send({status:"error",message:"product already exists"})};
        const products=await getProductService();
